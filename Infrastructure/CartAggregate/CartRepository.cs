...
namespace Infrastructure.CartAggregate
{
    ...
    {
        public override Task<Cart?> GetByIdAsync(int id, CancellationToken cancellationToken)
            => _context.Carts
                .Include(x => x.Items)
                .FirstOrDefaultAsync(x => x.Id == id, cancellationToken);

        public Task<List<Cart>> GetCartsByProductId(int productId, CancellationToken cancellationToken)
            => _context.Carts
                .Include(x => x.Items)

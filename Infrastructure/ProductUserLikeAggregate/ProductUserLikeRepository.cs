...
namespace Infrastructure.ProductUserLikeAggregate
{
    ...
    {
        public Task<bool> ExistAsync(int userId, int productId, CancellationToken cancellationToken)
            => _context.ProductUserLikes.AnyAsync(x => x.UserId == userId && x.ProductId == productId,cancellationToken);

        public Task<ProductUserLike?> GetAsync(int userId, int productId, CancellationToken cancellationToken)
